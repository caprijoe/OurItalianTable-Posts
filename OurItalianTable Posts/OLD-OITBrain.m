...
-(NSMutableArray *)isFav:(BOOL)fav
    ...
    if (!fav) {
        ...
        for (OLDPostRecord *postRecord in [self.brainEntries reverseObjectEnumerator]) {
...
-(NSArray *)getFavorites
{
    ...
    for (OLDPostRecord *entry in self.brainEntries)
    {
...
-(void)populateIcon:(OLDPostRecord *)postRecord
    ...
    // check if icon is in cache, if found populate memory too
        ...
        OLDPostRecord *post = [self.brainEntries objectAtIndex:index];
